...
class Item:
    ...
    def display_names(self, surface, name, cost, selected):
        color_2 = TEXT_COLOR_SELECTED if selected else TEXT_COLOR

        # title
        title_surf = self.font.render(name, False, color_2)
        title_rect = title_surf.get_rect(midtop = self.rect.midtop + pygame.math.Vector2(0, 20))

        # cost 
        cost_surf = self.font.render(f'${int(cost)}', False, color_2)
        cost_rect = cost_surf.get_rect(midbottom = self.rect.midbottom + pygame.math.Vector2(0, -25))

        # draw
        surface.blit(title_surf, title_rect)
        color = "#333333"
        pygame.draw.rect(surface, color, cost_rect.inflate(45, 10), border_radius=1)
        surface.blit(cost_surf, cost_rect)
        pygame.draw.rect(surface, "#f2f2f2", cost_rect.inflate(45, 10), 1)
